...
// Fractal constructor with center passed --------------------------------------
Fractal::
...
Fractal::
Fractal(double df, double kf, double ol)
: _ol(ol),
  _df(df),
  _kf(kf),
  _a(mean_radius),
...
add_random_monomer() {
  ...
  for (Vector_3 a_center; overlap(new_monomer);) {
    new_monomer = random_vec();
    new_monomer *= 2 * _a * _ol; // Scale unit vector to twice monomer radius
...
bool Fractal::
overlap(const Vector_3& monomer) {
