...
main(int argc, char **argv) {
  ...
  if (p.check_df()) {
    df = p.get_df();
    validate_df(df, true);
    INFO << "CLI argument df=" << df;
  }else if (p.check_def()) {
    df = fractal_dimension;
    INFO << "Using default value for fractal dimension. df=" << df;
  }
  // Prefactor
  if (p.check_kf()) {
    kf = p.get_kf();
    validate_kf(kf, true);
    INFO << "CLI argument kf=" << kf;
  }else if (p.check_def()) {
    kf = prefactor;
    INFO << "Using default value for prefactor. kf=" << kf;
  }
  // Monomer Count
  if (p.check_n()) {
    n = p.get_n();
    validate_n(n, true);
    INFO << "CLI argument n=" << n;
  }else if (p.check_def()) {
    n = monomer_count;
    INFO << "Using default value for monomer count. n=" << n;
  }
  // Overlap Factor
  if (p.check_k()) {
    k = p.get_k();
    validate_k(k, true);
    INFO << "CLI argument k=" << k;
  }else if (p.check_def()) {
    k = overlap;
    INFO << "Using default value for overlap. k=" << k;
  }
  // Epsilon
  if (p.check_e()) {
    e = p.get_e();
    validate_e(e, true);
    INFO << "CLI argument e=" << e;
  }else if (p.check_def()) {
    e = epsilon;
    INFO << "Using default value for epsilon. e=" << e;
  }
