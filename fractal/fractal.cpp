...
monte_carlo() {
  ...
  double out_of_range = _n * _a;
  INFO << "Out of range best_monomer = " << out_of_range;
  ...
  // Random monomer to attach to.
    ...
    if (!under_epsilon && (best_diff / _a) < _e) {
      INFO << "Under epsilon in " << (attempt+1) << " attempts.";
      under_epsilon = true;
    }
	}
  ...
  if (actual_epsilon < _e) {
    INFO << "Actual epsilon = " << actual_epsilon;
    add_monomer(best_monomer);
  }else {
    FATAL << "Best monomer did not satisfy epsilon."
               << "\n  best_diff / _a = " << (best_diff / _a)
               << "\n  epsilon = " << _e;
  }
}
